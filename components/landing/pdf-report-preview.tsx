...
export function PdfReportPreview({ compact = false }: { compact?: boolean }) {
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-[1.7rem] border border-slate-300/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.99),rgba(244,247,251,0.98))] shadow-[0_36px_90px_-56px_rgba(15,23,42,0.42)]",
        compact
          ? "p-2.5 sm:p-3"
          : "p-3 sm:p-4"
      )}
    >
      <div className="overflow-hidden rounded-[1.35rem] border border-slate-200 bg-white shadow-[0_24px_60px_-40px_rgba(15,23,42,0.2)]">
        <div className="flex justify-center bg-slate-100/80 p-2.5 sm:p-3">
          <HtmlTemplatePreview
            html={previewReportHtml}
            title="Monthly PDF report preview"
            width={920}
            height={1280}
            scale={compact ? 0.63 : 0.69}
