...
function getReportContact(input: PdfContext) {
...
function drawPageChrome(input: {
  ...
  doc.text(input.footerText, MARGIN, PAGE_HEIGHT - 26, {
    maxWidth: CONTENT_WIDTH - 60
  });
  doc.text(`Page ${input.pageNumber}`, PAGE_WIDTH - MARGIN, PAGE_HEIGHT - 26, {
    align: "right"
  });

  const footerExtras = [input.footerWebsite, input.footerNote].filter(Boolean) as string[];
  if (footerExtras.length > 0) {
    let footerExtraY = PAGE_HEIGHT - 14 - (footerExtras.length - 1) * 10;

    doc.setFontSize(9);
    doc.setTextColor(100, 116, 139);
    footerExtras.forEach((line) => {
      doc.text(line, PAGE_WIDTH / 2, footerExtraY, {
        align: "center",
...
export async function renderAiReportPdf(input: PdfContext) {
  ...
  const footerText = `Report generated by ${brandName} using SitePulse's client reporting system. Next automated report: ${getNextReportDate(input)}. Questions? Reply to ${getReportContact(input)}.`;
  ...
  const contactCopy = [
    ...
    `Contact email: ${getReportContact(input)}`,
    footerWebsite ? `Agency website: ${footerWebsite}` : null,
    `Website monitored: ${input.website.url}`,
    `Next automated report: ${getNextReportDate(input)}`,
    footerNote ? `Footer note: ${footerNote}` : null,
    "Questions? Reply to this email and your agency team can help with the next steps."
  ]
    .filter(Boolean)
    .join("\n");
  const contactY = tipY + tipHeight + 26;
  const contactHeight = Math.max(168, 56 + estimateTextHeight(doc, contactCopy, CONTENT_WIDTH - 40, 20));
  if (contactY + contactHeight > PAGE_HEIGHT - 50) {
    startNewPage("Mobile vs Desktop", "Agency contact");
    drawRoundedCard(doc, MARGIN, 124, CONTENT_WIDTH, contactHeight, [255, 255, 255]);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(15);
    doc.setTextColor(15, 23, 42);
    doc.text("Agency contact", MARGIN + 20, 152);
    drawMultilineText({
      doc,
      text: contactCopy,
      x: MARGIN + 20,
  ...
  drawRoundedCard(doc, MARGIN, contactY, CONTENT_WIDTH, contactHeight, [255, 255, 255]);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(15);
  doc.setTextColor(15, 23, 42);
  doc.text("Agency contact", MARGIN + 20, contactY + 28);
  drawMultilineText({
    doc,
    text: contactCopy,
    x: MARGIN + 20,
