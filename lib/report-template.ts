...
function centeredLogo(context: ReportContext) {
  if (!context.agency_logo_url) return "";
  return `<div style="width:100%;display:flex;justify-content:center;align-items:center;overflow:visible;margin-bottom:12px"><img src="${esc(context.agency_logo_url)}" alt="${esc(context.agency_name)}" style="width:clamp(120px, 20%, 220px);height:auto;object-fit:contain;display:block;max-width:none;filter:brightness(0) invert(1)"/></div>`;
}
