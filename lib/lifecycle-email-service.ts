...
type LifecycleProfile = Pick<
  UserProfile,
  "id" | "email" | "full_name" | "trial_ends_at" | "is_trial" | "created_at" | "email_notifications_enabled"
>;
...
type ActivationProfile = Pick<
  UserProfile,
  "id" | "email" | "full_name" | "created_at" | "is_trial" | "trial_ends_at" | "email_notifications_enabled"
>;
...
export type LifecycleEmailType =
  ...
  | "onboarding_day_5"
  | "onboarding_day_7"
  | "trial_day_1_scan_summary"
  | "trial_day_7_growth_plan"
  | "trial_day_13_expiry";

export type ActivationEmailType = Extract<
  LifecycleEmailType,
  "activation_day1" | "activation_day3" | "activation_day7"
>;
...
function canReceiveLifecycleEmail(
  profile: Pick<UserProfile, "email" | "email_notifications_enabled"> | null | undefined
) {
...
async function getActivationProfileById(
  ...
  const { data, error } = await admin
    .from("users")
    .select("id,email,full_name,created_at,is_trial,trial_ends_at,email_notifications_enabled")
    .eq("id", userId)
...
async function maybeSendWelcomeEmails(profile: LifecycleProfile, snapshot: LifecycleSnapshot, sent: string[]) {
  if (!canReceiveLifecycleEmail(profile) || getDaysSince(profile.created_at) > 7) {
    return;
  }
  ...
  await queueEmail(
    ...
    sendProductEmail({
      ...
      details: [
        {
          label: "Trial ends",
          value: formatLifecycleDate(profile.trial_ends_at)
        },
        {
          label: "Days left",
...
async function maybeSendTrialReminderEmails(profile: LifecycleProfile, sent: string[]) {
  if (!canReceiveLifecycleEmail(profile) || !profile.is_trial || !profile.trial_ends_at) {
    return;
  }
...
async function maybeSendOnboardingEmails(profile: LifecycleProfile, snapshot: LifecycleSnapshot, sent: string[]) {
  if (!canReceiveLifecycleEmail(profile)) {
    return;
  }
...
function buildTrialIssueList(topIssues: TrialScanSummary["topIssues"]) {
  const issues = topIssues.slice(0, 3);

  if (!issues.length) {
    return `
      <p style="margin:0 0 14px 0;font-size:15px;line-height:24px;color:#475569;">
        Your first scan finished cleanly, and there were no obvious critical issues in the first pass. Keep scanning so SitePulse can catch regressions early and build useful before-and-after proof.
      </p>
    `;
  }
  ...
  return `
    <p style="margin:0 0 14px 0;font-size:15px;line-height:24px;color:#475569;">
      Here are the three issues that stood out first:
    </p>
    <ul style="margin:0 0 16px 18px;padding:0;color:#475569;font-size:15px;line-height:24px;">
      ${issues
        .map(
          (issue) => `
            <li style="margin:0 0 10px 0;">
              <strong>${escapeHtml(issue.title)}</strong>${issue.description ? `: ${escapeHtml(issue.description)}` : ""}
            </li>
          `
        )
        .join("")}
...
export async function sendDay1Email(user: TrialLifecycleProfile, scanSummary: TrialScanSummary) {
  if (!canReceiveLifecycleEmail(user)) {
    return null;
  }

  const admin = createSupabaseAdminClient();
  const dedupeKey = buildEmailDedupeKey("lifecycle", "trial_day_1_scan_summary", user.id);
  const delivery = await sendProductEmail({
    templateId: "trial_day_1_scan_summary",
    dedupeKey,
    campaign: "trial_lifecycle",
    to: user.email,
    subject: `Here's what we found on ${scanSummary.domain}`,
    preheader: `Your first SitePulse scan for ${scanSummary.domain} is ready with the top issues to fix first.`,
    eyebrow: "Trial day 1",
    title: `Here's what we found on ${scanSummary.domain}`,
    summary: "Your first scan is complete. Start with the biggest issues first so the dashboard becomes useful immediately.",
    bodyHtml: `
      ${buildTrialIssueList(scanSummary.topIssues)}
      <p style="margin:0;font-size:15px;line-height:24px;color:#475569;">
        The SitePulse team
      </p>
    `,
    ctaLabel: "View your full dashboard",
    ctaUrl: getDashboardUrl("/dashboard"),
    details: [
      {
        label: "Domain",
        value: scanSummary.domain
      },
      {
        label: "Top issues",
        value: String(Math.min(scanSummary.topIssues.length, 3))
      }
    ],
    metadata: {
      userId: user.id,
      websiteId: scanSummary.websiteId,
      scanId: scanSummary.scanId
    },
    triggeredAt: scanSummary.scannedAt
  });

  await markLifecycleEmailSent(admin, user.id, "trial_day_1_scan_summary");
  return delivery;
}

export async function sendDay7Email(user: TrialLifecycleProfile) {
  if (!canReceiveLifecycleEmail(user)) {
    return null;
  }
  ...
  const delivery = await sendProductEmail({
    templateId: "trial_day_7_growth_plan",
    dedupeKey,
    campaign: "trial_lifecycle",
    to: user.email,
    subject: "Half your trial is gone — here's what you're missing",
    preheader: "Growth unlocks branded PDFs, daily scans, and scheduled delivery once you're ready to move past the trial.",
    eyebrow: "Trial day 7",
    title: "Half your trial is gone — here's what you're missing",
    summary: "The Growth plan turns SitePulse from a one-off audit tool into a repeatable client delivery system.",
    bodyHtml: `
      <p style="margin:0 0 14px 0;font-size:15px;line-height:24px;color:#475569;">
        <strong>PDF reports:</strong> Send polished, branded audits that look like part of your agency service instead of a raw export.
      </p>
      <p style="margin:0 0 14px 0;font-size:15px;line-height:24px;color:#475569;">
        <strong>Daily scans:</strong> Keep every client site monitored without manually rerunning checks whenever a question comes in.
      </p>
      <p style="margin:0;font-size:15px;line-height:24px;color:#475569;">
        <strong>Scheduled delivery:</strong> Queue reports automatically so your team keeps proving value even when nobody has time to assemble an update by hand.
      </p>
    `,
    ctaLabel: "Upgrade now — founding price locks in today",
    ctaUrl: getDashboardUrl("/pricing"),
    metadata: {
      userId: user.id
    },
  ...
  await markLifecycleEmailSent(admin, user.id, "trial_day_7_growth_plan");
  return delivery;
}

export async function sendDay13Email(user: TrialLifecycleProfile) {
  if (!canReceiveLifecycleEmail(user)) {
    return null;
  }
  ...
  const delivery = await sendProductEmail({
    templateId: "trial_day_13_expiry",
    dedupeKey,
    campaign: "trial_lifecycle",
    to: user.email,
    subject: "Your trial ends tomorrow",
    preheader: "Your founding price disappears when the trial expires. Upgrade before midnight to keep it.",
    eyebrow: "Trial day 13",
    title: "Your trial ends tomorrow",
    summary: "This is the last day to lock in the founding price before it expires with your trial.",
    bodyHtml: `
      <p style="margin:0;font-size:15px;line-height:24px;color:#475569;">
        Your trial ends tomorrow, and the founding price disappears when it expires. Upgrade before midnight to keep the lower rate.
      </p>
    `,
    ctaLabel: "Upgrade before midnight",
    ctaUrl: getDashboardUrl("/pricing"),
    accent: "#B91C1C",
    metadata: {
      userId: user.id
  ...
  await markLifecycleEmailSent(admin, user.id, "trial_day_13_expiry");
  return delivery;
}
...
async function sendActivationEmailBatch(
  ...
  const pendingProfiles = profiles.filter(
    (profile) =>
      canReceiveLifecycleEmail(profile) &&
      !alreadySentUserIds.has(profile.id) &&
      !userIdsWithWebsites.has(profile.id)
  );
...
export async function processLifecycleEmailsBatch(input: { limit?: number; offset?: number }) {
  ...
  const { data: users, error } = await admin
    .from("users")
    .select("id,email,full_name,trial_ends_at,is_trial,created_at,email_notifications_enabled")
