...
import { escapeHtml, getReportEmailMeta } from "@/lib/email-utils";
...
export type ReportEmailTemplateResult = {
  fromName: string;
  replyTo?: string;
  subject: string;
  preheader: string;
  accent: string;
  meta: ReturnType<typeof getReportEmailMeta>;
  html: string;
};
  ...
  if (input.securityHeaders) {
    ...
    if (presentCount < 6) {
      issues.push({
        ...
        title: "Your site is missing key security protections",
        subtitle:
          `${6 - presentCount} security headers are still missing, which weakens trust signals and browser-level protection.`,
        learnMoreUrl: "https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers",
...
function buildIssueSummaries(input: {
  ...
  const deduped = Array.from(new Map(allIssues.map((issue) => [issue.title, issue])).values());
...
export function buildReportEmailTemplate(input: ReportEmailTemplateInput): ReportEmailTemplateResult {
  const baseUrl = ensureHttpsUrl(input.baseUrl ?? getBaseUrl()).replace(/\/$/, "");
  ...
  const manageReportsUrl = `${baseUrl}/dashboard/reports`;
